...
from agent.action_system import Action_System
...
class RandomAgent:
    ...
    def pick_action(self, env, player):
        valid_moves = env.get_valid_moves()
        if not valid_moves:
            return None
        return random.choice(valid_moves)

def evaluate_model_vs_random(policy_net, device, num_games, logger):
    ...
    with torch.no_grad():
        for _ in range(num_games):
            ...
            current_player = 1  # Let DQN always be Player 1
            while not done:
                if current_player == 1:
                    # DQN Agent's turn
                    state_tensor = torch.from_numpy(env.get_board()).float().unsqueeze(0).to(device)
                    q_values = policy_net(state_tensor)
                    action = torch.argmax(q_values).item()
                else:
                    # Random Agent's turn
                    action = random_agent.pick_action(env, current_player)
                ...
                done = env.is_draw() or env.check_winner() != 0
    ...
    return {"dqn_wins": dqn_wins, "random_wins": random_wins, "draws": draws}
...
def main():
    try:
        ...
        agent_mcts = ActionSystem(policy_net)
    ...
    for episode in range(1, NUM_EPISODES + 1):
        ...
        logger.info(
            ...
            f"P2 (Random) wins={win_count_p2}, Total_Reward2={total_reward2}, "
            f"Draws={draw_count}, Result={winner_str}"
        )

        # Evaluation Phase
        if episode % EVAL_INTERVAL == 0:
            eval_results = evaluate_model_vs_random(policy_net, device, EVAL_EPISODES, logger)
